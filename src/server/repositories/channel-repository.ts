 ...
 * Channel Repository
 *
 * The ONLY interface for accessing channel-scoped data.
...
/**
 * ChannelRepository - Enforces channel scoping on ALL queries
 *
 * This class is the ONLY way to access channel-scoped data.
 * Every method automatically includes channel_id filtering.
 *
 * SECURITY: Never bypass this class for direct database queries.
 *
 * @example
 * ```typescript
 * const repo = new ChannelRepository(db, channelId);
 * const videos = await repo.getVideos({ status: 'scripting' });
 * const video = await repo.getVideo(videoId); // Returns null if wrong channel
 * ```
 */
export class ChannelRepository {
  ...
  constructor(db: NodePgDatabase<typeof schema>, channelId: string) {
    if (!channelId) {
      throw new Error('ChannelRepository requires a channelId');
    }
    this.db = db;
  ...
  /**
   * Get the channel ID this repository is scoped to
   */
  getChannelId(): string {
  ...
  async getVideos(options?: VideoListOptions): Promise<Video[]> {
    ...
    const conditions: SQL[] = [eq(videos.workspaceId, this.channelId)];
   ...
   * Returns null if video doesn't exist or belongs to a different channel
   */
  async getVideo(id: string): Promise<Video | null> {
    const result = await this.db
      .select()
      .from(videos)
      .where(and(eq(videos.id, id), eq(videos.workspaceId, this.channelId)))
      .limit(1);
  ...
  /**
   * Create a new video in the channel
   */
  async createVideo(
   ...
   * Returns true if deleted, false if not found or wrong channel
   */
   ...
   * Note: Documents are scoped via their video's channel
   */
  async getDocuments(options?: DocumentListOptions): Promise<Document[]> {
    ...
    // Documents are channel-scoped through their videos
    // We need to join with videos to ensure channel isolation
    return this.db
      .select({
        id: documents.id,
   ...
   * Returns null if document doesn't exist or belongs to a different channel's video
   */
  async getDocument(id: string): Promise<Document | null> {
    const result = await this.db
      ...
      .from(documents)
      .innerJoin(videos, eq(documents.videoId, videos.id))
      .where(and(eq(documents.id, id), eq(videos.workspaceId, this.channelId)))
      .limit(1);
  ...
  async getDocumentsByVideo(videoId: string): Promise<Document[]> {
    // First verify the video belongs to this channel
   ...
   * Throws if video doesn't exist or belongs to a different channel
   */
  ...
  async updateDocument(
    ...
    // First verify the document belongs to a video in this channel
  ...
  /**
   * Get all categories in the channel
   */
  async getCategories(options?: CategoryListOptions): Promise<Category[]> {
    ...
    return this.db
      .select()
      .from(categories)
      .where(eq(categories.workspaceId, this.channelId))
      .orderBy(orderFn(orderColumn))
      .limit(limit);
  }
   ...
   * Returns null if category doesn't exist or belongs to a different channel
   */
  async getCategory(id: string): Promise<Category | null> {
    const result = await this.db
      ...
      .from(categories)
      .where(
        and(eq(categories.id, id), eq(categories.workspaceId, this.channelId))
      )
      .limit(1);
  ...
  /**
   * Create a new category in the channel
   */
  ...
  async setVideoCategories(
    ...
    // Verify all categories belong to this channel using a single batch query
    if (categoryIds.length > 0) {
      const existingCategories = await this.db
        ...
        .where(
          and(
            eq(categories.workspaceId, this.channelId),
  ...
  async addVideoCategory(videoId: string, categoryId: string): Promise<void> {
    ...
    // Verify the category belongs to this channel
  ...
  /**
   * Get audit log entries for this channel
   */
  async getAuditLog(options?: PaginationOptions): Promise<AuditLogEntry[]> {
    ...
    return this.db
      .select()
      .from(auditLog)
      .where(eq(auditLog.workspaceId, this.channelId))
  ...
  async getAuditLogForEntity(
    ...
    return this.db
      ...
      .where(
        and(
          eq(auditLog.workspaceId, this.channelId),
   ...
   * @throws Error if document not found or belongs to different channel
    ...
    const result = await this.db
      ...
      .where(
        and(
          ...
          eq(videos.workspaceId, this.channelId)
        )
      )
      .limit(1);
  ...
  async restoreDocumentRevision(
    ...
    // Verify the document belongs to this channel
    const document = await this.getDocument(documentId);
    if (!document) {
      throw new Error('Document not found or access denied');
    }
 ...
 * @param channelId - The channel ID to scope all queries to
 */
export function createChannelRepository(
  db: NodePgDatabase<typeof schema>,
  channelId: string
): ChannelRepository {
  return new ChannelRepository(db, channelId);
}
